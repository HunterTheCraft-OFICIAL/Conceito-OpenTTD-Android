# 📄 src/blitter/32bpp_optimized.cpp - Tradução Conceitual

> **Nota:** Este é um documento conceitual em português do Brasil baseado no arquivo original `src/blitter/32bpp_optimized.cpp` do OpenTTD.
> 
...
uint r = remap[GB(m, 0, 8)];
uint r = remap[GB(m, 0, 8)];
template void Blitter_32bppOptimized::Draw<true>(Blitter::BlitterParams *bp, BlitterMode mode, ZoomL
template void Blitter_32bppOptimized::Draw<false>(Blitter::BlitterParams *bp, BlitterMode mode, Zoom
...
- **Arquivo Original:** `src/blitter/32bpp_optimized.cpp`
